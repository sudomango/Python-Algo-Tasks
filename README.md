...
18\. Количество вхождений элемента в последовательность: в текстовом файле записана произвольная строка или текст. Пользователь вводит символ или подстроку. Необходимо вывести, какое количество раз она повторяется в исходном тексте. Поиск сделать case-insensitive (регистронезависимый).
...
20\. Реализовать функции dec_to_hex, hex_to_dec, dec_to_bin, bin_to_dec, rgb_to_hex, hex_to_rgb без использования встроенных функций и методов языка программирования для перевода и парсинга. Проверить правильность работы этих функций на примерах.
